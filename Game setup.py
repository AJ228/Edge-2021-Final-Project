...
def PlayRPS(player,comp):
    Rps = ["Rock","Paper","Scissors"] #borrowing bits from player setup for turn testing
    PRPS = random.choice(Rps) #Rps = rock-paper-scissors
    CRPS = random.choice(Rps)

    while PRPS == CRPS:
        PRPS = random.choice(Rps)
        CRPS = random.choice(Rps) 
